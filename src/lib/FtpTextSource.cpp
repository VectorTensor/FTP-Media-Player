...
tl::expected<std::string, std::string> test_ftp_text_source() {
  std::cout << "FtpTextSource stub running. curl version: " << curl_version()
            << "\n";
...
tl::expected<FtpClient *, std::string>
create_ftp_client(const std::string &host, const std::string &username,
...
void apply_common_opts(const FtpClient &client) {
  curl_easy_setopt(client.curl_handler, CURLOPT_USERNAME,
                   client.config.username.c_str());
  curl_easy_setopt(client.curl_handler, CURLOPT_PASSWORD,
...
tl::expected<void, std::string> ftp_connect(const FtpClient &client) {
  ...
  curl_easy_setopt(client.curl_handler, CURLOPT_URL,
...
tl::expected<std::vector<ResourceInfo>, std::string>
ftp_list_resources(const FtpClient &client, std::string path) {

  std::string listing;
  apply_common_opts(client);
  std::string url_path = client.config.host + path;
  curl_easy_setopt(client.curl_handler, CURLOPT_URL, url_path.c_str());
  ...
  curl_easy_setopt(client.curl_handler, CURLOPT_VERBOSE, 1L);
  std::cout<<"url path" <<url_path<<std::endl;

  CURLcode res = curl_easy_perform(client.curl_handler);
  if (res != CURLE_OK) {
    return tl::unexpected(std::string("FTP list failed: ") +
